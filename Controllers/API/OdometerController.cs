...
namespace CarCareTracker.Controllers
{
    public partial class APIController
    {
        ...
        public IActionResult AllOdometerRecords(MethodParameter parameters)
        {
            ...
            var result = vehicleRecords.Select(x => new OdometerRecordExportModel { VehicleId = x.VehicleId.ToString(), Id = x.Id.ToString(), Date = x.Date.ToShortDateString(), InitialOdometer = x.InitialMileage.ToString(), Odometer = x.Mileage.ToString(), Notes = x.Notes, ExtraFields = x.ExtraFields, Files = x.Files, Tags = string.Join(' ', x.Tags), EquipmentRecordId = string.Join(' ', x.EquipmentRecordId), Source = x.Source.ToString() });
