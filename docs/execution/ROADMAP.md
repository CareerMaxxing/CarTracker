...
| 14 | V1 Hardening | Unit/integration/UI/migration/duplicate-operation/adapter-failure/backup-restore tests, error handling, accessibility, responsive validation, performance, security review. | 🟡 In progress — Increment 1 (security review) complete: found and fixed a real static-file auth bypass + unrestricted file-type upload. Increment 2 (automated tests) complete: xUnit + WebApplicationFactory test project stood up (deferred since Phase 7), 10 passing tests covering Phases 7/9/12/14's highest-risk flows (PHASE_14.md). Accessibility/mobile/performance still open |
...
- Phase 14 Increment 2 (Automated Tests): user chose this next; stood up the xUnit/WebApplicationFactory project deferred since Phase 7, 10 tests passing covering the highest-risk flows identified across prior phases. ✅ done. **← next: more Phase 14 increments (accessibility/mobile/performance) or user's next priority**
