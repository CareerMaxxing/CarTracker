...
| 17 | Real MOT History & Advisory Tracking | Pull the vehicle's full MOT history (not just the latest test), extract every advisory/failure across all past tests into tracked Planner items with recurring-advisory detection and a lighter "mark resolved" status. Revisits CLAUDE.md's "mocked DVLA/DVSA adapters only" locked decision with the human owner's explicit sign-off — switches to real DVSA MOT History API data. | 🟡 All 6 increments complete, not yet deployed to production — credential plumbing (1), real DVSA adapter with a live end-to-end failure-path proof against Microsoft's OAuth endpoint (2), full MOT history UI (3), advisory text normalization (4), Add-to-Planner with recurring advisories visually grouped (5, verified live against a real vehicle), and an orthogonal "mark resolved" status that caught and fixed a real null-safety bug along the way (6). Full loop verified end-to-end against real dev data; production deployment is the next step (PHASE_17.md) |
...
- Phase 17 (Real MOT History & Advisory Tracking): user asked to expand MOT tracking (full history,
  ...
  (6). Full loop verified end-to-end against real dev data. **Not yet deployed to production** -
  deliberately held back until the full loop was complete rather than shipping a half-finished
  feature; this is the next step. Full increment-by-increment detail in `PHASE_17.md`.
