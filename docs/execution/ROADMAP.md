...
| 18 | Post-Deployment Functionality Review | User reported a brand-new feature (MOT Ignore) as completely non-functional and asked directly whether shipped work was actually being verified - go through real workflows and the codebase to confirm what's genuinely functional, not just claimed. | 🟡 In progress — found and fixed the real root cause of the Ignore report: static-asset cache-busting had been broken app-wide the entire session (StaticHelper.VersionNumber never changed across dozens of deploys, so browsers never re-fetched updated JS). Increment 1 (Kanban board headers unreadable/undersized - real CSS bug, fixed: min-width + horizontal scroll instead of force-shrinking lanes, header font rescoped) complete and deployed. Two further findings from real-data investigation, not yet actioned - need the user's direction: Parts vs Supplies is a confirmed unfinished Phase 5 migration (the docs say so directly), not a false impression; the Calendar tab is legitimately empty (zero reminders configured), and the real timeline the user asked for already exists on the vehicle page but excludes PlanRecords (PHASE_18.md) |
