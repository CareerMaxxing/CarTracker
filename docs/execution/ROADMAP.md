...
| 17 | Real MOT History & Advisory Tracking | Pull the vehicle's full MOT history (not just the latest test), extract every advisory/failure across all past tests into tracked Planner items with recurring-advisory detection and a lighter "mark resolved" status. Revisits CLAUDE.md's "mocked DVLA/DVSA adapters only" locked decision with the human owner's explicit sign-off — switches to real DVSA MOT History API data. | 🟡 In progress — Increment 1 (DVSA credential plumbing) complete. Increment 2 (RealDVSAAdapter: OAuth2 client-credentials + the real MOT History API call, live-config-selected with transparent mock fallback) complete — real end-to-end failure-path proof against Microsoft's actual OAuth endpoint. Increment 3 (full MOT history UI: every past test rendered, colour-coded advisories, split Mock badges) complete, curl-verified against both real dev vehicles. Increment 4 (advisory text normalization + per-vehicle dedup key, built before the Planner-linkage actions that depend on it per a Plan-agent review) complete, unit-tested. Increment 5 (Add to Planner + bulk import, deduped, recurring advisories visually grouped) complete — verified live against a real vehicle whose recurring advisories collapsed into single rows exactly as the user asked (PHASE_17.md) |
